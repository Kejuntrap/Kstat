...
def posts(content):
    ...
    key = os.environ.get("SECURE").encode("utf-8")
    signature = hmac.new(key,content,hashlib.sha1).hexdigest()
    data = content
    site = 'https://q.trap.jp/api/v3/webhooks/752025fd-30c0-4dcb-841e-b5962f0bee32'     #main
    debug = 'faaf39df-1e8e-4fce-b33b-d5b13c7af686'
    #headers = {'content-type': 'text/plain' , 'X-TRAQ-Signature': f'{signature}',}
    headers = {'content-type': 'text/plain' , 'X-TRAQ-Signature': f'{signature}','X-TRAQ-Channel-Id': f'{debug}',}  #デバッグヘッダ
    response = requests.post(site, headers=headers, data=data)
